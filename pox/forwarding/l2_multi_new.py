...
class Switch (EventMixin):
  ...
  def _handle_PacketIn (self, event):
    log.info("XXXXX PACKET IN with in_port: %d", event.port)
    if event.port == 65535:
      ...
      if core.openflow_discovery.is_edge_port(loc[0].dpid, loc[1]):
        ...
        if packet.dst in mac_map:
          ...
          log.warning("Packet from %s to known destination %s arrived "
                      "at %s.%s without flow", packet.src, packet.dst,
                      dpidToStr(self.dpid), event.port)
...
class l2_multi (EventMixin):
  ...
  def _handle_BarrierIn (self, event):
    log.info("XXXX Barrier IN %s", event.xid)
    wp = waiting_paths.pop((event.dpid,event.xid), None)
    log.info("XXX path is %s", wp)
    if not wp:
