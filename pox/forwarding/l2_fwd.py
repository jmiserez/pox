...
class S1(EventMixin):
  ...
  def get_dst_h1(self, event):
    ...
    self.log.info("XXX s1 dest h1 Output with buffer_id: %d", event.ofp.buffer_id)
    return s1_msg
...
class S2(EventMixin):
  ...
  def get_dst_h1(self, event):
    ...
    self.log.info("XXX s2 dest h1 Output with buffer_id: %d", event.ofp.buffer_id)
    return s1_msg, s2_msg
  ...
  def get_dst_h2(self, event):
    ...
    s2_msg.actions.append(of.ofp_action_output(port=hosts_map[s2][h2]))
    s2_msg.buffer_id = event.ofp.buffer_id
    self.log.info("XXX s2 dest h2 Output with buffer_id: %d", event.ofp.buffer_id)
    return s2_msg
